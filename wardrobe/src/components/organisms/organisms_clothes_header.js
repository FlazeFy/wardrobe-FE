    ...
    return (
        <>
            <div className={`box-clothes ${props.type == 'schedule' ? 'p-2': props.type == 'clothes-outfit' ? 'px-3 py-2 text-center':''}`} onClick={(e) => (props.handleClick ? props.handleClick() : handleBoxClick(props.items.id, props.type))}>
                ...
                {
                    props.type == 'clothes' && <a className='btn btn-success btn-add-used-history w-100' data-bs-toggle="modal" data-bs-target={"#addHistoryModal_"+props.items.id}><FontAwesomeIcon icon={faPlus}/> Use It Now!</a>
                }
            </div>
            {
